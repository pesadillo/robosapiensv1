...
// Send Robosapien V2 sequence
void IRsend::sendRSV2(unsigned long data)
{
...
int nbits=RSV2_BITS;
	...
	// This sequence is quite simple - 12 bits - intial pulse seems to vary a little but does not affect the seq
...
int IRrecv::decode(decode_results *results) {
  ...
  if (decodeRC6(results)) {
	...
	#ifdef DEBUG
		Serial.println("Attempting ROBOSAPIEN decode");
	#endif 
	
    if (decodeRSV1(results)) {
        return DECODED;
    }
...
long IRrecv::decodeRSV1(decode_results *results) {
  ...
  if (!MATCH_MARK(results->rawbuf[offset], RS_HDR_SPACE)) {
    return ERR;
  }
   
  // Check raw length valuues
  if (irparams.rawlen < 2 * RSV1_BITS + 2) {
    return ERR;
  }
  ...
  for (int i = 0; i < RSV1_BITS-1; i++) {
  
    if (!MATCH_MARK(results->rawbuf[offset], RS_BIT_MARK)) {
      return ERR;
    }
    offset++;
    if (MATCH_SPACE(results->rawbuf[offset], RS_ONE_SPACE)) {
      data = (data << 1) | 1;
	
    } 
    else if (MATCH_SPACE(results->rawbuf[offset], RS_ZERO_SPACE)) {
      data <<= 1;
